...
fn benchmark_hft_data_structures(c: &mut Criterion) {
    ...
    group.bench_function("order_book_top_access", |b| {
        b.iter(|| black_box(order_book.front()))
    });
    ...
    let price_levels: Vec<(f64, f64)> = (0..100).map(|i| (100.0 + i as f64 * 0.01, 10.0)).collect();
    ...
    group.bench_function("binary_search_price_insertion", |b| {
        b.iter(|| {
            ...
            let result = price_levels
                .binary_search_by(|&(price, _)| price.partial_cmp(&target_price).unwrap());
            black_box(result)
        })
    });

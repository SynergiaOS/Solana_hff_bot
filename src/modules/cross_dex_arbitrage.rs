...
impl CrossDexArbitrageStrategy {
    ...
    pub async fn update_price_data(&mut self, token_mint: String, price_data: PriceData) -> Result<()> {
        // Update price matrix
        let dex_prices = self.price_matrix.entry(token_mint.clone()).or_insert_with(BTreeMap::new);
    ...
    async fn scan_cross_dex_opportunities(&mut self, token_mint: &str) -> Result<()> {
        ...
        for i in 0..dex_list.len() {
            for j in (i + 1)..dex_list.len() {
                ...
                if let (Some(price_a), Some(price_b)) = (dex_prices.get(dex_a), dex_prices.get(dex_b)) {
                    // Check both directions
                    self.evaluate_arbitrage_direction(token_mint, price_a, price_b).await?;
                    self.evaluate_arbitrage_direction(token_mint, price_b, price_a).await?;
                }
            }
        }
    ...
    async fn evaluate_arbitrage_direction(
        ...
        let arbitrage_path = self.create_arbitrage_path(
            token_mint,
            buy_price,
            sell_price,
            max_trade_size,
        ...
        let opportunity = CrossDexOpportunity {
            ...
            risk_score,
            estimated_execution_time_ms: self.estimate_execution_time(&buy_price.dex, &sell_price.dex),
        ...
        if confidence_score >= 0.7 {
            info!("🎯 Cross-DEX Arbitrage: {:.2}% profit on {} ({:?} -> {:?})", 
    ...
    fn calculate_risk_score(&self, path: &ArbitragePath, buy_price: &PriceData, sell_price: &PriceData) -> f64 {
        let mut risk = 0.0;
        ...
        let now = Utc::now();
        let max_age = (now - buy_price.timestamp).max(now - sell_price.timestamp).num_seconds();
        if max_age > 10 {
            risk += 0.2;
        }
    ...
    fn meets_risk_tolerance(&self, complexity: &ExecutionComplexity, risk_score: f64) -> bool {
        match self.config.risk_tolerance {
            ...
            RiskTolerance::Moderate => {
                matches!(complexity, ExecutionComplexity::Simple | ExecutionComplexity::Medium) 
                && risk_score < 0.5
            }
    ...
    fn calculate_confidence_score(
        ...
        let now = Utc::now();
        let max_age = (now - buy_price.timestamp).max(now - sell_price.timestamp).num_seconds() as f64;
    ...
    async fn create_trading_signal(&self, opportunity: &CrossDexOpportunity) -> Result<TradingSignal> {
    ...
    pub async fn cleanup_expired_opportunities(&mut self) {
        ...
        if removed_count > 0 {
            debug!("Cleaned up {} expired cross-DEX opportunities", removed_count);
        }
    }
}

...
impl AIConnector {
    pub async fn new(
        ...
        let client = Client::open(&config.dragonfly_url)?;
    ...
    pub async fn start(&mut self) -> Result<()> {
        ...
        let brain_listener = self.start_brain_listener();
        let market_event_processor = self.start_market_event_processor();
        let health_monitor = self.start_health_monitor();

        // Run all tasks concurrently
        tokio::try_join!(brain_listener, market_event_processor, health_monitor)?;
    ...
    async fn listen_for_ai_decisions(
        ...
        let result: Option<(String, String)> = conn
            .blpop("overmind:trading_commands", self.config.brain_request_timeout.as_secs())
            .await?;
        ...
        if let Some((_, decision_json)) = result {
            ...
            info!("🧠 Received AI decision: {} {} (confidence: {:.2f})", 
    ...
    async fn process_ai_decision(
        ...
        if ai_decision.confidence < self.config.confidence_threshold {
            warn!("Rejecting low-confidence AI decision: {:.2f} < {:.2f}", 
    ...
    async fn check_brain_health(&self, conn: &mut ConnectionManager) -> Result<bool> {
        ...
        let response: Option<(String, String)> = conn
            .blpop("overmind:health_response", 5) // 5 second timeout
            .await?;
